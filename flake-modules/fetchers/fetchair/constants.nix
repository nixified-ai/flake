...
let
  findSingleAttr =
    p: none: more: attrs:
    lib.lists.findSingle (set: p set.name set.value) none more (lib.attrsToList attrs);
in
rec {
  modelTypeFromName =
    name:
    lib.lists.findSingle (k: v: v.name == name) null
  ...
  ecosystemFromName =
    ...
    findSingleAttr (
      k: v: v.name == name
    ) null (lib.throw "ecosystemFromName: more than one ecosystem with the name ${name}") ecosystems;
  ...
  ecosystemOf =
    ...
    if isNull base then
      ...
      lib.lists.findSingle (ecosystem: lib.lists.elem base ecosystem.models) default
