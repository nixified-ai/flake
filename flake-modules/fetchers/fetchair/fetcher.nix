...
let
  ...
  url =
    if
      isNull parsed.version
    # not sure why the spec has version as optional when civitai's download urls seemingly rely on it
    then
      throw "a model version is required (...:<model>@<version>) in order to fetch this AI resource"
    else if parsed.source == "civitai" then
      # ex:
      ...
      + (params [
        (
          if parsed.type == constants.modelTypes.vae.urn then "type=VAE" else "type=Model" # don't know what other cases there are to handle
        )
        (optStrFn (s: "format=${s}") parsed.format) # necessary?
      ])
    else if parsed.source == "huggingface" then
      # ex:
      ...
      let
        ...
        resource =
          lib.optionalString (subdir != "") "${subdir}/"
          + parsed.version
          + optStrFn (s: ".${s}") parsed.format;
      in
      "https://${parsed.source}.co/${owner}/${repo}/resolve/main/${resource}?download=true"
    else
  ...
  metaAttr = {
    ...
    // (
      if (parsed.source == "civitai") then
        {
          homepage = "https://${parsed.source}.com/models/${parsed.model}?modelVersionId=${parsed.version}";
        }
      else if (parsed.source == "huggingface") then
        { homepage = lib.strings.replaceStrings [ "resolve/" "?download=true" ] [ "blob/" "" ] url; }
      else
        { }
    );
  };
