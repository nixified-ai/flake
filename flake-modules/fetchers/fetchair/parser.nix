...
urn:
let
  orNull = p: x: if p x then x else null;
  parts =
    let
...
in
{
  ecosystem = lib.lists.findSingle (
    x: x.urn == part 0
  ) null (throw "more than one ecosystem with the name ${part 0}") (builtins.attrValues ecosystems);
  type = lib.lists.findSingle (
    x: x.urn == part 1
  ) null (throw "more than one type with the name ${part 1}") (builtins.attrValues modelTypes);
  source = part 2;
  model = part 3;
  version = part 4;
  format = part 5;
