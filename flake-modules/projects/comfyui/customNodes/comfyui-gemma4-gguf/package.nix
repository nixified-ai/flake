...
}:
{
  ...
  postPatch = ''
    substituteInPlace gemma4_nodes.py \
      ...
      --replace-fail 'full_model_path = os.path.join(folder_paths.models_dir, rel_model_dir, model_file)' 'full_model_path = folder_paths.get_full_path("LLM", model_file) or os.path.join(folder_paths.models_dir, rel_model_dir, model_file)' \
      --replace-fail 'full_mmproj_path = os.path.join(folder_paths.models_dir, rel_model_dir, mmproj_file)' 'full_mmproj_path = folder_paths.get_full_path("LLM", mmproj_file) or os.path.join(folder_paths.models_dir, rel_model_dir, mmproj_file)'
