...
#              nix-store --load-db < ${closure}/registration
              ...
              do
                ...
                if ! nix build --option trusted-substituters daemon --option substituters daemon --option build-users-group "" --timeout 5000 ${inputs.self}#macos-ventura-image --keep-failed -L
                then
                  upload_failure
