...
let
  ...
  ''
    ...
    exec ${vncdoWrapper} "Select Your Country or Region" type "united states" pause 1 key shift-tab pause 1 key space
    expect "Written and Spoken Languages"
    exec ${vncdoWrapper} "Written and Spoken Languages" key shift-tab pause 1 key space
    expect "Accessibility"
    exec ${vncdoWrapper} "Accessibility" key shift-tab pause 1 key space
    expect "How Do You Connect?"
    exec ${vncdoWrapper} "How Do You Connect?" move 815 480 click 1 pause 1 move 1330 820 click 1
    expect "Your Mac isn't connected to"
    exec ${vncdoWrapper} "Your Mac isn't connected to" move 1020 640 click 1
    expect "Data & Privacy"
    exec ${vncdoWrapper} "Data & Privacy" key shift-tab pause 1 key space
    expect "Migration Assistant"
    exec ${vncdoWrapper} "Migration Assistant" key tab pause 1 key tab pause 1 key tab pause 1 key space

    ## Not needed when offline
    # expect "Sign In with Your Apple ID"
    # exec ${vncdoWrapper} "Sign in with Your Apple ID" key shift-tab pause 1 key shift-tab pause 1 key space
    # expect "Are you sure you want to skip"
    # exec ${vncdoWrapper} "Are you sure you want to skip signing in with an Apple ID?" key tab pause 1 key space
    # expect "Terms and Conditions"

    exec ${vncdoWrapper} "Terms and Conditions" key shift-tab pause 1 key space
    ...
    exec ${vncdoWrapper} "Are you sure you don't want to use Location Services?" key tab pause 1 key space
    expect "Select Your Time Zone"
    exec ${vncdoWrapper} "Select Your Time Zone" key tab pause 1 type UTC pause 1 key enter pause 1 key shift-tab pause 1 key space
    expect "Analytics"
    exec ${vncdoWrapper} "Analytics" key tab pause 1 key space pause 1 key shift-tab pause 1 key space
    expect "Screen Time"
    exec ${vncdoWrapper} "Screen Time" key tab pause 1 key space
